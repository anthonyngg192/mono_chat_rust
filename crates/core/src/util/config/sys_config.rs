...
static CONFIG_BUILDER: Lazy<RwLock<Config>> = Lazy::new(|| {
    RwLock::new({
        let mut builder = Config::builder()
            .add_source(File::from_str(include_str!("env.json"), FileFormat::Json));

        if std::path::Path::new("env.json").exists() {
            builder = builder.add_source(File::new("env.json", FileFormat::Json));
        }
