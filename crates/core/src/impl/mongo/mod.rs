use crate::{util::manipulation::prefix_key, AbstractDatabase, Error, Result};
...
impl MongoDb {
    ...
    async fn find_with_option<O, T: DeserializeOwned + Unpin + Send + Sync>(
        &self,
        collection: &'static str,
        projection: Document,
        options: O,
    ...
    async fn find<T: DeserializeOwned + Unpin + Send + Sync>(
        ...
        self.find_with_option(collection, projection, None).await
    }
