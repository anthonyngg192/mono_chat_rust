...
use crate::{
    models::{
        message::{AppendMessage, MessageQuery, MessageSort, MessageTimePeriod, PartialMessage},
        Message,
    },
    r#impl::mongo::DocumentId,
    AbstractMessage, Error, Result,
};
        ...
        if let Some(embeds) = &append.embeds {
            if !embeds.is_empty() {
                query.insert(
                    ...
                    doc! {
                        "embeds":{
                            "$each":to_bson(embeds) .map_err(|_| Error::DatabaseError {
                                operation: "to_bson",
                                with: "embeds"
                            })?
                        }
    ...
    async fn fetch_messages(&self, query: MessageQuery) -> Result<Vec<Message>> {
        ...
        match query.time_period {
            MessageTimePeriod::Relative { nearby } => {
                ...
                let (a, b) = try_join!(
                    self.find_with_option::<_, Message>(
                        COL,
