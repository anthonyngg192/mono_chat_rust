...
pub async fn req(
    ...
    permissions
        .throw_permission_and_view_channel(db, ChannelPermission::SendMessage)
        .await?;
    ...
    if let Some(embeds) = edit.embeds {
        ...
        permissions
            .throw_permission_and_view_channel(db, ChannelPermission::SendEmbeds)
            .await?;
    ...
    if permissions
        .has_permission(db, ChannelPermission::SendEmbeds)
        .await?
    {
        if let Some(content) = edit.content {
            chat_core::tasks::process_embeds::queue(
