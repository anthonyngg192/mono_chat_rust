    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            WSErrorType::TransportConnectionFailure => {
                write!(f, "An error occurred while trying to connect transport")
            }

            WSErrorType::ProducerFailure => write!(
                f,
                "An unknown error occurred while setting up an RTC producer"
            ),
            WSErrorType::ProducerNotFound => write!(f, "Producer doesn't exist"),

            WSErrorType::ConsumerFailure => write!(
                f,
                "An unknown error occurred while setting up an RTC consumer"
            ),
